 ...
 * the butterflyChatbotConfig should be defined in the html file before this script is included
 * the butterflyChatbotConfig should contain the token of the chatbot
...
(function () {
  ...
  const configKey = "butterflyChatbotConfig";
  const buttonId = "butterfly-chatbot-bubble-button";
  const iframeId = "butterfly-chatbot-bubble-window";
  ...
  const originalIframeStyleText = `
    ...
    top: unset;
    right: var(--${buttonId}-right, 1rem); /* Align with butterfly-chatbot-bubble-button. */
    bottom: var(--${buttonId}-bottom, 1rem); /* Align with butterfly-chatbot-bubble-button. */
    left: unset;
    width: 24rem;
  ...
  async function embedChatbot() {
    ...
    const baseUrl =
      config.baseUrl || `https://${config.isDev ? "dev." : ""}ubutterfly.app`;
    ...
    function createIframe() {
      ...
      iframe.title = "butterfly chatbot bubble window";
      iframe.id = iframeId;
    ...
    window.addEventListener('message', (event) => {
      ...
      if (event.data.type === 'butterfly-chatbot-iframe-ready') {
        targetIframe.contentWindow?.postMessage(
          {
            type: 'butterfly-chatbot-config',
            payload: {
      ...
      if (event.data.type === 'butterfly-chatbot-expand-change') {
        toggleExpand();
      }
    });
