...
const PluginPage = ({
  ...
  document.title = `${t('plugin.metadata.title')} - Butterfly`
  ...
  const handleFileChange = (file: File | null) => {
    if (!file || !file.name.endsWith('.butterflypkg')) {
      setCurrentFile(null)
      return
    }
          ...
          <div className='flex shrink-0 items-center gap-1'>
            {
              isExploringMarketplace && (
                <>
                  <Link
                    href={`https://docs.butterfly.ai/${locale === LanguagesSupported[1] ? 'v/zh-hans/' : ''}plugins/publish-plugins/publish-to-butterfly-marketplace`}
                    target='_blank'
                  >
                    <Button
                      className='px-3'
