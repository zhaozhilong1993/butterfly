...
const Header: FC<IHeaderProps> = ({
  ...
  const handleMessageReceived = useCallback((event: MessageEvent) => {
    let currentParentOrigin = parentOrigin
            if (!currentParentOrigin && event.data.type === 'butterfly-chatbot-config') {
      currentParentOrigin = event.origin
      setParentOrigin(event.origin)
    }
    if (event.origin !== currentParentOrigin)
      return
            if (event.data.type === 'butterfly-chatbot-config')
  ...
  useEffect(() => {
    ...
    window.addEventListener('message', listener)
            ...
            window.parent.postMessage({ type: 'butterfly-chatbot-iframe-ready' }, '*')
  ...
  const handleToggleExpand = useCallback(() => {
    ...
    window.parent.postMessage({
              type: 'butterfly-chatbot-expand-change',
    }, parentOrigin)
